<script>
  ...
  export default {
    ...
    created: function() {
        this.retrieveDateData()
    },
    
    methods: {
        async retrieveDateData() {
                this.dateData = await getTodayDate()
        }
    }
}
...
<template>
  ...
  <div class ="site-header">
      ...
      <span class="today-date" v-if="$data.dateData.hasOwnProperty(0)">
         <a href="#my-sign">･｡ﾟ My Sign</a> | <a href="#all-signs">.･｡ All Signs</a> | <b>Today's date : </b>{{dateData}}</span>
      </div> 
  </div> 
